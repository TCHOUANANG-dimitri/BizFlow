...
import { pull as apiPull, push as apiPush, PushBody } from './api';
...
class SyncEngine {
  ...
  private async pushOutbox(): Promise<number> {
    ...
    const body: PushBody = { sales: [], money_movements: [], stock_movements: [], daily_closings: [] };
    ...
    for (const item of pending) {
      try {
        ...
        body[byKind(item.kind)].push(payload);
      } catch {
    ...
    for (const r of all) {
      if (r.status === 'accepted' || r.status === 'duplicate') {
        await markOutboxSynced(r.client_uuid);
        pushed += 1;
      } else if (r.status === 'rejected') {
        await markOutboxRejected(r.client_uuid, r.detail ?? 'Rejeté par le serveur');
      }
    }
    return pushed;
  }
